  ...
  <div>
    <div v-for="(slice, index) in slices" :key="'slice-' + index">
      <template v-if="slice.slice_type === 'feature_person'">
        <section class="blk-featperson__wrapper section-wrapper" :style="{ backgroundColor: slice.primary.featured_person_color }">
                ...
                <div class="inview-content inview-full bottom">
                    <div class="container">
                        <div class="row blk-featperson__title">
                            <div class="col-sm-12">
                                <h1>
                                <span class="author">{{ $prismic.richTextAsPlain(slice.primary.featured_person_author) }}</span>
                                <span class="title">{{ $prismic.richTextAsPlain(slice.primary.featured_person_title) }}</span>
                                </h1>
                            </div>
                        </div>
            ...
            <div class="inview-wrapper blk-featpost">
                ...
                <div class="inview-content inview-full">
                    ...
                    <div class="row bottom blk-featpost__cover__title">
                        <div class="col-sm-12">
                            <h2>
                                <span class="title">{{ $prismic.richTextAsPlain(slice.primary.timeline_title) }}</span>
                            </h2>
                        </div>
                    </div>
        ...
        <section class="blk-featpost__wrapper section-wrapper">
            <div class="inview-wrapper blk-featpost">
                ...
                <div class="container">
                    ...
                    <div class="col-sm-12">
                        <h2>
                            <span class="title">{{ $prismic.richTextAsPlain(slice.primary.featured_post_title) }}</span>
  ...
  methods: {
    getContent() {
        ...
        this.$prismic.client.query(
            this.$prismic.Predicates.at('document.type', 'timeline'),
            { orderings : '[my.timeline.order]' }).then((response) => {
